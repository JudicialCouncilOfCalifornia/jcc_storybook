Drupal.behaviors.name751={attach:function attach(){(function(a){"complete"===document.readyState||"interactive"===document.readyState?setTimeout(a,1):document.addEventListener("DOMContentLoaded",a)})(function(){// Helper function to get the Visible pixel height of an element.
...
var i=Array.from(document.querySelectorAll(".sidebar-nav"));i.forEach(function(a){var b=a.getElementsByClassName("sidebar-nav__position");if(0<b.length){var c=b[0].clientHeight;c>=a.parentNode.clientHeight?a.parentNode.style.minHeight=c+"px":a.parentNode.style.removeProperty("minHeight")}});var j=document.querySelectorAll(".footer"),k=document.querySelectorAll(".shoe");(j[0]||k[0])&&["load","scroll","resize"].forEach(function(b){return window.addEventListener(b,function(){// Get the total footer visible in pixels. This includes footer and shoe.
var b=k[0]?a(k[0]):0,c=j[0]?a(j[0]):0,d=c+b,e=Array.from(document.querySelectorAll("body:not(.mobile-sidebar-nav) .sidebar-nav"));e.forEach(function(a){var b=a.getElementsByClassName("sidebar-nav__position");if(0<b.length&&(b[0].style.removeProperty("max-height"),b[0].style.removeProperty("overflow"),0<d))// We only adjust the sidebar height if the footer becomes visible.
{d=0<d?d+40:d;// Factor the max-height based on window height, minus the footer
// height, minus any visible space on the top of the sidebar nav.
var c=window.innerHeight-d-a.getBoundingClientRect().top;// If the avaialable space is 0 or less, set the max height to 0,
// essentially hiding the sidebar nav.
0>=c&&(c=0),b[0].style.maxHeight=c+"px",b[0].style.overflow="auto"}})})})})}};