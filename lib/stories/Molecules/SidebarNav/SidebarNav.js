Drupal.behaviors.name353={attach:function attach(){/**
...
return b(h)}(function(a){"complete"===document.readyState||"interactive"===document.readyState?setTimeout(a,1):document.addEventListener("DOMContentLoaded",a)})(function(){// Apply class to body if sidebar nav is present.
...
var i=Array.from(document.querySelectorAll(".sidebar-nav"));i.forEach(function(a){var b=a.getElementsByClassName("sidebar-nav__position");if(0<b.length){var c=b[0].clientHeight;c>=a.parentNode.clientHeight?a.parentNode.style.minHeight=c+"px":a.parentNode.style.removeProperty("minHeight")}});// Set the max height of the sidebar nav, to fit within any available space.
// This only renders on desktop (non-mobile) display, on load, scroll and 
// resize events.
var j=document.querySelectorAll(".footer"),k=document.querySelectorAll(".shoe");["load","scroll","resize"].forEach(function(b){return window.addEventListener(b,function(){// Get the total footer visible in pixels. This includes footer and shoe.
var b=a(j[0])+a(k[0]),c=Array.from(document.querySelectorAll(".sb-main-fullscreen:not(.mobile-sidebar-nav) .sidebar-nav"));c.forEach(function(a){var c=a.getElementsByClassName("sidebar-nav__position");if(0<c.length&&(c[0].style.removeProperty("max-height"),c[0].style.removeProperty("overflow"),0<b))// We only adjust the sidebar height if the footer becomes visible.
