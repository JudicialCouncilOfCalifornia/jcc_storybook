Drupal.behaviors.name308={attach:function attach(){function a(a){// Migrate to new heading tags.
...
for(d&&d.split(" ").forEach(function(a){a.trim()&&e.classList.add(a)});a.firstChild;)e.appendChild(a.firstChild);a.parentNode.replaceChild(e,a)})})}function b(a,b){// Extend cards component styling - see Section.css.
...
if(!b.classList.contains("custom")){var c=b.querySelectorAll(".accordion, .action-list, .body, .cards, .steps, .teaser-list, .profile-cards, .view-results .content, .tabs, .list");if(c){// Determine if section heading exists.
var d=b.querySelector(".section__content > .container > .section__header"),e=null!==d&&d.querySelector("h1, h2")?d.querySelector("h1, h2").tagName:null;e&&"H1"!==e&&(a(c),b.classList.add("custom"))}}})}// Adjust for cards support.
